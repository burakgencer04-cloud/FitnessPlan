...
import { useShallow } from 'zustand/react/shallow'; // 🔥 FIX: Shallow eşitlik için eklendi
...
export function useProfile() {
  ...
  // 🔥 FIX: 8 Ayrı Subscription tek bir useShallow çağrısında birleştirildi (Performans Zırhı)
  const { 
    user, setUser, setMacros, setCustomTargetMacros, 
    setCustomWorkouts, setMealPlan, logout, 
    activeThemeId, setActiveThemeId 
  } = useAppStore(useShallow(state => ({
    user: state.user || {}, // Güvenli default ataması
    setUser: state.setUser,
    setMacros: state.setMacros,
    setCustomTargetMacros: state.setCustomTargetMacros,
    setCustomWorkouts: state.setCustomWorkouts,
