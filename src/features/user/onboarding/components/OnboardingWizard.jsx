...
export default function OnboardingWizard({ onComplete, themeColors: C }) {
  ...
  const handleNext = () => {
    if (step === 1 && (!form.firstName.trim() || !form.lastName.trim() || !form.city.trim())) return alert(t('wiz_err_personal') || "Tüm kişisel bilgileri doldurun.");
    if (step === 2 && (!form.gender || !form.dobDay || !form.dobMonth || !form.dobYear)) return alert(t('wiz_err_demographics') || "Cinsiyet ve doğum tarihini seçin.");
    if (step === 3 && (!form.height || !form.startWeight || !form.targetWeight)) return alert(t('wiz_err_body') || "Tüm vücut ölçülerini girin.");
    if (step === 4 && (!form.goal || !form.activity)) return alert(t('wiz_err_goal') || "Hedefinizi ve aktivite seviyenizi seçin.");
    if (step === 5 && !form.dietType) return alert(t('wiz_err_diet') || "Bir beslenme tarzı seçin.");
    if (step === 6 && (!form.experience || !form.days)) return alert(t('wiz_err_workout') || "Spor geçmişi ve gün sayısını seçin.");
    if (step === 8 && !form.waterGoalLiters) return alert(t('wiz_err_water') || "Su hedefini girin.");
 ...
 const finalizeSetup = () => {
    ...
    // 1. ID ile ara -> 2. Gün sayısıyla ara -> 3. Sonuncu programı ver
    ...
    const finalData = {
      ...
      // 🚀 KULLANICIYA ÖZEL PROGRAMI KAYDET
