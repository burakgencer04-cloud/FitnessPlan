...
export default function Onboarding({ onDone, theme, existingUser }) {
  ...
  // 🔥 AKILLI PROGRAM ATAMA ALGORİTMASI
  // 🔥 YENİLMEZ AKILLI PROGRAM ATAMA ALGORİTMASI 🔥
  const handleFinishSetup = () => {
    // 1. Kullanıcının seçtiği gün ve tecrübeyi al (Farklı değişken isimlerini garantiye alıyoruz)
    const targetDays = parseInt(form.trainDays) || parseInt(form.days) || parseInt(form.workoutsPerWeek) || 3;
    const exp = form.experience || "baslangic"; 

    // 2. İdeal ID'yi belirle
    let targetId = "fb_3";
    ...
    // 3. İlk deneme: Direkt ID ile bul (Eğer workoutData.js'de eklediysen)
    ...
    if (!matchedPlan) {
      matchedPlan = WORKOUT_PRESETS.find(p => p.name && p.name.includes(`${targetDays} Gün`));
    }
    ...
    // 8. Veriyi paketle ve gönder
    const finalData = {
      ...form,
      activePlanId: matchedPlan?.id || "default",
      activePlanName: matchedPlan?.name || "Özel Program",
      customWorkouts: finalWorkouts // Antrenman sekmesi DOĞRUDAN bu diziyi okuyacak!
    };

    onDone(finalData); // Sistemi ateşle!
  };
