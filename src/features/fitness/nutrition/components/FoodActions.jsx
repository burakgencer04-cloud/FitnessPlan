...
export default function FoodItemRow({ item, idx, isLast, handleToggleEaten, handleDeleteFood, C = {} }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', padding: "12px 0", borderBottom: !isLast ? `1px solid rgba(255,255,255,0.04)` : "none", gap: 12 }}>
      <button 
        onClick={() => handleToggleEaten?.(item)} 
        style={{ width: 28, height: 28, borderRadius: '50%', flexShrink: 0, cursor: 'pointer', transition: 'all 0.25s ease', border: `2px solid ${item?.isEaten ? (C?.green || '#22c55e') : 'rgba(255,255,255,0.2)'}`, background: item?.isEaten ? (C?.green || '#22c55e') : 'rgba(0,0,0,0.3)', color: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: item?.isEaten ? `0 0 15px rgba(46, 204, 113, 0.4)` : 'none' }}
      >
      ...
      <div style={{ flex: 1, opacity: item?.isEaten ? 1 : 0.6, transition: '0.3s' }}>
        ...
        <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", fontWeight: 700, display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
          ...
          <span>
            <span style={{ color: C?.green || '#22c55e' }}>{item?.p}P</span> • 
            <span style={{ color: C?.yellow || '#f59e0b' }}>{item?.c}C</span> • 
            <span style={{ color: '#a855f7' }}>{item?.f}Y</span>
          </span>
      ...
      <button 
        ...
        style={{ background: 'none', border: 'none', color: C?.red || '#ef4444', fontSize: 18, cursor: 'pointer', padding: "4px", opacity: 0.5, transition: "0.2s" }} 
