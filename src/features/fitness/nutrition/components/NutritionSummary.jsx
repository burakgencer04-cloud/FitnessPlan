...
// 🔥 ZIRH: Her render'da yeniden yaratılmayan statik (sabit referanslı) stil objeleri
const S = {
  macroBar: { position: 'relative', width: '100%', height: 4, background: 'rgba(255,255,255,0.05)', borderRadius: 10, overflow: 'hidden' },
  ...
  circleInner: { width: 64, height: 64, borderRadius: '50%', background: "rgba(20, 20, 25, 0.9)", display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', boxShadow: "inset 0 4px 10px rgba(0,0,0,0.4)" },
  cardBase: { padding: "20px", marginBottom: 20 },
  rowBetween: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  colRight16: { flex: 1, paddingRight: 16 },
  flexCenterGap8: { display: "flex", alignItems: "center", gap: 8 },
  flexBaselineGap6: { display: "flex", alignItems: "baseline", gap: 6 },
  grid12: { display: 'grid', gap: 12 },
  grid2Col: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, paddingTop: 16 },
  statBox: { padding: "12px 16px", boxShadow: "0 4px 10px rgba(0,0,0,0.1)" },
  waterHeader: { display: "flex", justifyContent: "space-between", alignItems: "flex-end" },
  waterControls: { display: "flex", gap: 10, marginTop: 4 },
  actionCols: { display: "flex", flexDirection: "column", gap: 12, marginBottom: 20 },
  toolBtnInner: { display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", textAlign: "left" },
  iconBox: { textAlign: "center", width: 44, height: 44, borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }
};
...
function MacroBar({ label, plannedVal = 0, eatenVal = 0, target = 1, color }) {
  ...
  return (
    <div>
      <div style={{ ...S.rowBetween, fontSize: 11, fontWeight: 800, marginBottom: 6 }}>
      ...
      <div style={{ ...S.macroBar, border: `1px solid rgba(255,255,255,0.02)` }}>
...
export default function NutritionSummary({ plannedTotals = {}, eatenTotals = {}, targetMacros = {}, targetFiber = 30, targetSugar = 50, targetWater = 3000, waterConsumed = 0, conicGradient, handleAddWater, handleRemoveWater, WATER_PER_BOTTLE = 250, setShowAIVision, handleGenerateSamplePlan, C = {}, t = (k, fb)=>fb||k }) {
  ...
  return (
    <>
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} style={{ ...glassCardStyle, ...S.cardBase }}>
        <div style={{ ...S.rowBetween, marginBottom: 20 }}>
          <div style={S.colRight16}>
            <div style={{ ...S.flexCenterGap8, marginBottom: 6 }}>
              <div style={{ fontSize: 10, color: "rgba(255,255,255,0.5)", fontWeight: 900, letterSpacing: 1.5 }}>{t('nut_taken_cal')}</div>
            </div>
            <div style={{ ...S.flexBaselineGap6, marginBottom: 4 }}>
          ...
          <div style={{ ...S.circleWrap, background: conicGradient }}>
            <div style={{ ...S.circleInner, border: `1px solid rgba(255,255,255,0.05)` }}>
        ...
        <div style={{ ...S.grid2Col, borderTop: `1px solid rgba(255,255,255,0.06)` }}>
           <div style={{ ...glassInnerStyle, ...S.statBox }}>
      ...
      {/* SU TAKİBİ */}
      <div style={{ ...glassCardStyle, ...S.cardBase, display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={S.actionCols}>
        <motion.button onClick={() => setShowAIVision?.(true)} whileTap={{ scale: 0.98 }} style={{ ...glassCardStyle, ...S.cardBase, ...S.toolBtnInner, padding: 20, marginBottom: 0, background: "linear-gradient(145deg, rgba(139, 92, 246, 0.05), rgba(0,0,0,0.4))", border: `1px solid rgba(139, 92, 246, 0.15)` }}>
          <div>
          ...
          <div style={{ ...S.iconBox, background: "rgba(139, 92, 246, 0.1)", border: `1px solid rgba(139, 92, 246, 0.2)`, boxShadow: "0 0 15px rgba(139, 92, 246, 0.2)" }}>📸</div>
        </motion.button>

        <motion.button onClick={handleGenerateSamplePlan} whileTap={{ scale: 0.98 }} style={{ ...glassCardStyle, ...S.cardBase, ...S.toolBtnInner, padding: 20, marginBottom: 0 }}>
          <div>
          ...
          <div style={{ ...S.iconBox, background: "rgba(0,0,0,0.4)", border: `1px solid rgba(255,255,255,0.05)`, color: "#fff" }}>✨</div>
        </motion.button>
      </div>
    </>
  );
