  ...
  <AnimatePresence>
    {show && (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: 'fixed', inset: 0, zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
        <div style={{ position: "absolute", inset: 0, background: "rgba(5,8,12,0.5)", backdropFilter: "blur(16px)" }} onClick={onClose} />
        <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} onClick={e => e.stopPropagation()} style={{ background: `linear-gradient(145deg, ${C.card}E6, ${C.bg}CC)`, backdropFilter: "blur(24px)", borderRadius: 32, padding: 32, width: '100%', maxWidth: 400, border: `1px solid ${C.border}80`, boxShadow: "0 30px 60px rgba(0,0,0,0.5)", position: "relative", zIndex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
            <div>
              <h3 style={{ margin: 0, fontFamily: fonts.header, fontSize: 20, fontWeight: 900, color: C.text }}>Kayıt Ekle</h3>
              <div style={{ fontSize: 11, color: C.bg, background: C.text, fontWeight: 800, marginTop: 6, textTransform: 'uppercase', letterSpacing: 1, display: 'inline-block', padding: '4px 10px', borderRadius: 8 }}>{MEASUREMENT_TYPES.find(t => t.id === form.type)?.label.split(" ")[0]}</div>
            </div>
            <button onClick={onClose} style={{ background: "rgba(255,255,255,0.05)", border: `1px solid ${C.border}80`, color: C.text, width: 36, height: 36, borderRadius: 12, cursor: 'pointer', fontWeight: 900 }}>✕</button>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ background: `rgba(0,0,0,0.2)`, border: `1px solid ${C.blue}40`, padding: 16, borderRadius: 16, display: "flex", gap: 12, alignItems: "flex-start" }}>
              ...
              <div><div style={{ fontSize: 11, fontWeight: 900, color: C.blue, textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 }}>İPUCU</div><div style={{ fontSize: 12, color: C.sub, lineHeight: 1.5 }}>{MEASURE_TIPS[form.type]}</div></div>
            </div>
          </div>
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={onSave} style={{ width: "100%", marginTop: 24, padding: "16px", borderRadius: 16, background: C.text, border: "none", color: C.bg, fontWeight: 900, fontFamily: fonts.header, fontSize: 14, cursor: "pointer", boxShadow: `0 8px 20px rgba(0,0,0,0.3)` }}>KAYDET</motion.button>
        </motion.div>
      </motion.div>
    )}
  </AnimatePresence>
  ...
  <AnimatePresence>
    {index !== null && photos[index] && (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: 'fixed', inset: 0, zIndex: 10001, display: 'flex', flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20 }}>
        <div style={{ position: "absolute", inset: 0, background: "rgba(5,8,12,0.7)", backdropFilter: "blur(24px)" }} />
        <div style={{ width: "100%", maxWidth: 520, position: "relative", zIndex: 1 }}>
          <button onClick={() => setIndex(null)} style={{ position: "absolute", top: -40, right: 0, background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "#fff", width: 32, height: 32, borderRadius: "50%", fontSize: 14, cursor: "pointer", zIndex: 10, display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
          <motion.div key={index} initial={{ opacity: 0, scale: 0.95, x: 50 }} animate={{ opacity: 1, scale: 1, x: 0 }} exit={{ opacity: 0, scale: 0.95, x: -50 }} transition={{ type: "spring", damping: 25, stiffness: 200 }} drag="x" dragConstraints={{ left: 0, right: 0 }} dragElastic={0.5} onDragEnd={(e, info) => { if (info.offset.x > 50 && index > 0) setIndex(index - 1); else if (info.offset.x < -50 && index < photos.length - 1) setIndex(index + 1); }} style={{ background: `linear-gradient(145deg, ${C.card}E6, ${C.bg}E6)`, backdropFilter: "blur(24px)", borderRadius: 32, overflow: "hidden", border: `1px solid ${C.border}80`, boxShadow: "0 30px 60px rgba(0,0,0,0.5)", width: "100%" }} onClick={e => e.stopPropagation()}>
            <div style={{ width: "100%", aspectRatio: "3/4", background: "#000", position: "relative" }}>
              ...
              <div style={{ position: "absolute", top: 16, left: 16, background: "rgba(0,0,0,0.5)", color: "#fff", padding: "6px 12px", borderRadius: 20, fontSize: 11, fontWeight: 800, fontFamily: fonts.mono, backdropFilter: "blur(4px)" }}>{photos[index].date}</div>
            </div>
            <div style={{ padding: 24 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                <div><div style={{ fontSize: 12, color: C.sub, fontWeight: 700, letterSpacing: 1, marginBottom: 4 }}>O GÜNKÜ HACİM</div><div style={{ fontSize: 20, color: C.text, fontWeight: 900, fontFamily: fonts.mono }}>{(photos[index].volume / 1000).toFixed(1)} <span style={{ fontSize: 12, color: C.mute }}>ton</span></div></div>
              ...
              <button onClick={() => alert("Instagram'da paylaşmak için ekran görüntüsü alın!")} style={{ width: "100%", background: `linear-gradient(135deg, #f09433, #e6683c, #dc2743, #cc2366, #bc1888)`, color: "#fff", border: "none", padding: "16px", borderRadius: 16, fontWeight: 900, fontSize: 14, cursor: "pointer", display: "flex", justifyContent: "center", gap: 8, alignItems: "center", fontFamily: fonts.header, boxShadow: "0 10px 20px rgba(0,0,0,0.3)" }}><span>📸</span> Hikayede Paylaş</button>
            </div>
          </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: 'fixed', inset: 0, zIndex: 10002, display: 'flex', flexDirection: "column", alignItems: 'center', justifyContent: 'center', padding: 20 }}>
        ...
        <div style={{ width: "100%", maxWidth: 360, aspectRatio: "9/16", background: `linear-gradient(145deg, ${C.card}E6, ${C.bg}E6)`, borderRadius: 32, padding: 32, display: "flex", flexDirection: "column", justifyContent: "space-between", border: `1px solid ${C.border}80`, position: "relative", overflow: "hidden", zIndex: 1, boxShadow: "0 20px 50px rgba(0,0,0,0.5)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, position: "relative", zIndex: 2 }}>
            <div style={{ background: "rgba(0,0,0,0.3)", padding: 16, borderRadius: 16, border: `1px solid rgba(255,255,255,0.05)` }}><div style={{ fontSize: 24, fontWeight: 900, color: C.text, fontFamily: fonts.mono }}>{(globalTotalVolume / 1000).toFixed(1)}t</div><div style={{ fontSize: 10, color: C.sub, fontWeight: 700, letterSpacing: 1 }}>HACİM</div></div>
            <div style={{ background: "rgba(0,0,0,0.3)", padding: 16, borderRadius: 16, border: `1px solid rgba(255,255,255,0.05)` }}><div style={{ fontSize: 24, fontWeight: 900, color: C.text, fontFamily: fonts.mono }}>{personalRecords.length}</div><div style={{ fontSize: 10, color: C.sub, fontWeight: 700, letterSpacing: 1 }}>REKOR (PR)</div></div>
          </div>
