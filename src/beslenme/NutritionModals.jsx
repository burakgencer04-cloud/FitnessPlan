...
import GlassModalWrapper from '../components/GlassModalWrapper'; // 🚀 YENİ: İsim karışıklığını çözdük!
...
// 🎯 YENİ: GlassModalWrapper'a Bağlanmış Detay Modalı
export function FoodDetailModal({ food, onClose, onAddFood, C }) {
  ...
  useEffect(() => {
    if(food) { setCustomQty(food.qty || 100); setQtyUnit(food.unit === 'porsiyon' ? 'portion' : 'gram'); }
  }, [food]);

  const dRatio = qtyUnit === "portion" ? customQty : (customQty / (food?.qty || 1));
  const dCal = Math.round((food?.cal || 0) * dRatio); 
  const dP = ((food?.p || 0) * dRatio).toFixed(1);
  const dC = ((food?.c || 0) * dRatio).toFixed(1); 
  const dF = ((food?.f || 0) * dRatio).toFixed(1);

  return (
    <GlassModalWrapper isOpen={!!food} onClose={onClose} maxWidth={360} C={C}>
...
// 🎯 YENİ: GlassModalWrapper'a Bağlanmış Barkod Modalı
export function BarcodeScannerModal({ isOpen, onClose, onProductFound, C }) {
  ...
  const fetchProductFromAPI = async (barcode) => {
    ...
    try {
      ...
      } else { showAlert("Bulunamadı", "Bu barkoda ait ürün sistemde bulunamadı."); }
    } catch (error) { showAlert("Hata", "Bağlantı hatası oluştu."); } 
    finally { setIsFetchingBarcode(false); }
  };

  return (
    <GlassModalWrapper isOpen={isOpen} onClose={onClose} maxWidth={400} C={C}>
      ...
      <div style={{ padding: 20, textAlign: 'center', background: "rgba(0,0,0,0.4)" }}>
        <button onClick={onClose} style={{ background: "rgba(255,255,255,0.1)", color: C.text, padding: "12px 32px", borderRadius: 100, fontWeight: 900, border: "none", cursor: "pointer" }}>✕ İptal</button>
      </div>
    </GlassModalWrapper>
  );
}

// 🎯 YENİ: GlassModalWrapper'a Bağlanmış Örnek Plan Modalı
export function SamplePlanModal({ isOpen, onClose, activePlan, onApplySamplePlan, onApplyMealFromSample, DAYS, nutDay, C }) {
  return (
    <GlassModalWrapper isOpen={isOpen && activePlan && activePlan.meals} onClose={onClose} maxWidth={480} C={C}>
        ...
        {activePlan?.meals?.map((meal, mi) => (
          <div key={mi} style={{ background: "rgba(0,0,0,0.2)", borderRadius: 20, padding: 16, border: `1px solid rgba(255,255,255,0.05)`, boxShadow: "inset 0 2px 10px rgba(0,0,0,0.2)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, borderBottom: `1px dashed ${C.border}40`, paddingBottom: 16 }}>
              ...
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => onApplyMealFromSample(meal, mi)} style={{ background: `linear-gradient(135deg, ${C.green}20, transparent)`, color: C.green, border: `1px solid ${C.green}50`, padding: "8px 16px", borderRadius: 12, cursor: "pointer", fontWeight: 800, fontSize: 12, display: "flex", alignItems: "center", gap: 6, boxShadow: `0 4px 10px ${C.green}15` }}>Ekle <span>+</span></motion.button>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {meal.items.map((item, ii) => ( 
                <div key={ii} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 14, color: C.text }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}><div style={{ width: 6, height: 6, borderRadius: "50%", background: C.sub }}></div><span style={{ fontWeight: 600 }}>{item.name}</span></div>
      ...
      <div style={{ padding: 24, borderTop: `1px solid rgba(255,255,255,0.05)`, background: "rgba(0,0,0,0.3)" }}>
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={onApplySamplePlan} style={{ width: '100%', padding: "18px", borderRadius: 16, background: `linear-gradient(135deg, ${C.green}, #22c55e)`, color: "#000", fontWeight: 900, cursor: "pointer", border: "none", fontSize: 16, fontFamily: fonts.header, boxShadow: `0 10px 30px ${C.green}50`, display: "flex", justifyContent: "center", alignItems: "center", gap: 8 }}>Tümünü Plana Ekle</motion.button>
      </div>
    </GlassModalWrapper>
  );
}